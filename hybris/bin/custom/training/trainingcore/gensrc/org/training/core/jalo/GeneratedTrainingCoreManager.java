...
import org.training.core.jalo.CarWowUser;
...
public abstract class GeneratedTrainingCoreManager extends Extension
{
	...
	public CarWowUser createCarWowUser(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.CARWOWUSER );
			return (CarWowUser)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating CarWowUser : "+e.getMessage(), 0 );
		}
	}
	
	public CarWowUser createCarWowUser(final Map attributeValues)
	{
		return createCarWowUser( getSession().getSessionContext(), attributeValues );
	}
	...
	public Engine createEngine(final SessionContext ctx, final Map attributeValues)
	{
		try
		{
			ComposedType type = getTenant().getJaloConnection().getTypeManager().getComposedType( TrainingCoreConstants.TC.ENGINE );
			return (Engine)type.newInstance( ctx, attributeValues );
		}
		catch( JaloGenericCreationException e)
		{
			final Throwable cause = e.getCause();
		...
		{
			throw new JaloSystemException( e ,"error creating Engine : "+e.getMessage(), 0 );
		}
	}
	
	public Engine createEngine(final Map attributeValues)
	{
		return createEngine( getSession().getSessionContext(), attributeValues );
	}
